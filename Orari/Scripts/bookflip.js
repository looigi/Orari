...
function ini() {
    var widthPage = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
    var heightPage = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight;
    //alert(widthPage + 'x' + heightPage);
    pWidth = (widthPage / 2) - 45;
    pHeight = heightPage - 120;
	...
	for(var i=0;i<pagesDiv.childNodes.length;i++){
		if(pagesDiv.childNodes[i].nodeType == 1){ //element nodes only
			...
			document.getElementById(PREFISSO + "cphCorpo_bookflip").appendChild(newElement);

			if(j>0){createList(newElement,j);}; //create dropdown navigation list
			//createShadow(newElement, j); //shadow function

			pages[j] = document.getElementById(PREFISSO + "cphCorpo_bookflip").innerHTML;
			document.getElementById(PREFISSO + "cphCorpo_bookflip").removeChild(newElement);
			j++;
		}
	}

	if (myBook = document.getElementById(PREFISSO + "cphCorpo_bookflip")) {
...
function clipmeR(){
	...
	var CampoNascosto = document.getElementById(PREFISSO + "cphCorpo_hdnGiorno");
	CampoNascosto.value = selectIndex;

	ScriveRicorrenze();
}

function ScriveRicorrenze() {
    var CampoNascosto = document.getElementById(PREFISSO + "cphCorpo_hdnGiorno");
    Numero = CampoNascosto.value;
    var Inizio = 0;
    if (Numero == 0) { Inizio = 1; } else { Numero++; }

    var RicorrenzaS = document.getElementById(PREFISSO + "cphCorpo_lblRicorrenzaS");
    var RicorrenzaD = document.getElementById(PREFISSO + "cphCorpo_lblRicorrenzaD");

    var divRicorrenzaS = document.getElementById(PREFISSO + "cphCorpo_RicoS");
    var divRicorrenzaD = document.getElementById(PREFISSO + "cphCorpo_RicoD");

    if (Inizio == 1) {
        divRicorrenzaS.style.visibility ="hidden";
        Numero = 0;
    } else {
        if (typeof Ricorrenze[Numero] != 'undefined') {
            divRicorrenzaS.style.visibility = "visible";
            RicorrenzaS.innerHTML = Ricorrenze[Numero];
        } else {
            divRicorrenzaS.style.visibility = "hidden";
        }
    }
    if (typeof Ricorrenze[Numero + 1] != 'undefined') {
        divRicorrenzaD.style.visibility = "visible";
        RicorrenzaD.innerHTML = Ricorrenze[Numero + 1];
    } else {
        divRicorrenzaD.style.visibility = "hidden";
    }
}
...
function createShadow(dv,pg){
	...
	if(number_check(pg)){
		...
		//var myPngDiv2 = document.createElement('div');
		//myPngDiv2.style.position = "absolute";
		//myPngDiv2.style.top = '0px';
		//myPngDiv2.style.left = '0px';

		//myPngDiv2.style.width = (pageShadowWidth) + "px";
		//myPngDiv2.style.height = pHeight + "px";
		//myPngDiv2.style.backgroundColor = "#000000";

		//var ie6 = (/MSIE ((5\.5)|6)/.test(navigator.userAgent) && navigator.platform == "Win32");

		//if (ie6) {
		//    myPngDiv2.style.filter = "progid:DXImageTransform.Microsoft.Alpha(Opacity=" + pageShadowOpacity + ", FinishOpacity=0, Style=1, StartX=" + pageShadowWidth + ", FinishX=0, StartY=0, FinishY=0)";
		//} else {
		//    myPngDiv2.style.background = "transparent url(" + pageShadowRightImgUrl + ") top right repeat-y";
		//}
		//dv.appendChild(myPngDiv2);
    }
}
